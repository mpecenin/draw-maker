    ...
    */
    ...
    ns.BaseMaker = class {
        ...
        set(obj, property, value) {
            ...
            // Just to update the object bounding box, as needed.
            obj.setCoords();
    ...
    ns.ArrowMaker2 = class extends ns.BaseMaker {
        ...
        _arrowLength(x1, y1, x2, y2) {
            return this.roundFloat(Math.sqrt(Math.pow((x1 - x2), 2) + Math.pow((y1 - y2), 2)));
        }

        _arrowAngle(x1, y1, x2, y2) {
            return this.roundFloat(Math.atan2((y2 - y1), (x2 - x1)) * (180 / Math.PI));
        };

        _arrowPath(arrowLength, strokeWidth) {
            let s = strokeWidth + 4;
            let l = arrowLength - strokeWidth - s;
            l = (l > 0) ? l : 0;
            // Fix applied... pass as array.
            return [['m', 0, 0], ['h', l], ['v', -s], ['l', s, s], ['l', -s, s], ['v', -s], ['z']];
        }
        ...
        _next(options) {
            ...
            let arrowLength = this._arrowLength(this._startLeft, this._startTop, pointer.x, pointer.y);
            let arrowAngle = this._arrowAngle(this._startLeft, this._startTop, pointer.x, pointer.y);
            let arrowPath = this._arrowPath(arrowLength, strokeWidth);
            let shape = new fabric.Path(arrowPath, {
                drawmaker: { type: this.type },
                originX: 'left',
                originY: 'center',
                left: this._startLeft,
        ...
        update(options) {
            if (!this._attached) {
                this._shape = this._next(options)
                this.dm.canvas.add(this._shape);
                this._attached = true;
            } else {
                // Fix applied... remove/add.
    ...
    ns.DoubleArrowMaker2 = class extends ns.ArrowMaker2 {
        ...
        _arrowPath(arrowLength, strokeWidth) {
            let s = strokeWidth + 4;
            let l = arrowLength - strokeWidth - (s * 2);
            l = (l > 0) ? l : 0;
            return [['m', s, 0], ['v', -s], ['l', -s, s], ['l', s, s], ['v', -s], ['h', l], ['v', -s], ['l', s, s], ['l', -s, s], ['v', -s], ['z']];
        }
    ...
    ns.ArrowMaker = class extends ns.BaseMaker {
        ...
        set(obj, property, value) {
            ...
            // Just to update the group and objects bounding box, as needed.
            obj.addWithUpdate(); // TODO: isso está causando sumir objetos (ficar fora das coords do canvas), quando vários selecionados.
            this.dm.canvas.requestRenderAll();
        }
